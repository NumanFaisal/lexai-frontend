...
function DraftContent() {
  ...
  useEffect(() => {
    ...
    const draftDocs = documents.filter(d => d.mode === 'draft');
    if (docId && draftDocs.some((d) => d.id === docId)) {
      dispatch(setActiveDocument(docId));
    } else if (!activeDocumentId && draftDocs.length > 0) {
      dispatch(setActiveDocument(draftDocs[0].id));
    }
  ...
  // Only display draft documents
  const draftDocuments = documents.filter((d) => d.mode === 'draft');
          ...
          {showNewForm && (
            <div className="mt-3 rounded-xl border border-gold-border bg-bg-primary p-3">
              <textarea
                ...
                placeholder="Describe the contract you want to draft..."
                rows={3}
              ...
              <div className="w-full max-w-[800px] mx-auto flex items-center gap-3">
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    placeholder="Ask AI to revise this draft contract..."
                    type="text"
                  />
                </div>
                <button
          ...
          <div className="flex h-full items-center justify-center">
            <div className="text-center">
              ...
              <p className="mt-4 text-[14px] text-text-muted">
                Select a draft contract template or create a new one
              </p>
            </div>
          </div>
        )}
